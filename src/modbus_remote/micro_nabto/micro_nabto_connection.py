...
class RequestConnection(Request):
    device: DeviceInfo
    _data: ModbusDeviceInfo|None = None
    ...
    def __init__(self, sequence_id:int, device:DeviceInfo) -> None:
        ...
        self.device = device
    def set_data(self, value:ModbusDeviceInfo) -> None:
        self._data = value
        self.notify_waiters()
    def get_data(self) -> ModbusDeviceInfo|None:
        return self._data
...
class MicroNabtoConnection:
    ...
    async def connect(self, email:str, device_id:str, device_host:str|None=None, device_port:int|None=DEVICE_PORT, timeout:float = CONNECT_TIMEOUT) -> ModbusDeviceInfo|None:
        """
    ...
    def _process_device_info_payload(self, payload:bytes, request: RequestConnection) -> None:
        ...
        _LOGGER.debug(f"Successfully connected to device: {deviceinfo.device_id} with address: {deviceinfo.device_host}:{deviceinfo.device_port}. Device model: {deviceinfo.device_model}, device number: {deviceinfo.device_number}, slave device number: {deviceinfo.slave_device_number}, slave device model: {deviceinfo.slave_device_model}")
