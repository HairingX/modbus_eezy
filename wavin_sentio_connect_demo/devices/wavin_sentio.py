...
class WavinSentio(ModbusDeviceBase):
    def __init__(self, device_info: ModbusDeviceInfo):
        ...
        self._attr_datapoints = [
            ModbusDatapoint(key=WavinModbusDatapointKey.MODBUS_MODE, read_address=5, divider=1, signed=True, extra={"default_read": True}),
        ]
        self._attr_setpoints = [
            ModbusSetpoint(key=WavinModbusSetpointKey.MODBUS_PASSWORD, read_address=0, write_address=6, divider=1, min=1, max=65535, signed=True, extra={"default_read": True}),
        ]
